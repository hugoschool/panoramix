...
static void panoramix_initial_args(panoramix_t *panoramix, args_t *args)
{
...
static bool panoramix_init(panoramix_t *panoramix, args_t *args)
{
    panoramix_initial_args(panoramix, args);
    panoramix->villagers = villagers_init(args->nb_villagers, args->nb_fights);
    if (panoramix->villagers == NULL)
        return false;
    ...
    if (panoramix->servings_mutex == NULL || panoramix->refills_mutex == NULL
        || panoramix->villagers_sem == NULL || panoramix->druid_sem == NULL)
        return false;
    pthread_mutex_init(panoramix->servings_mutex, NULL);
    pthread_mutex_init(panoramix->refills_mutex, NULL);
    if (sem_init(panoramix->druid_sem, 0, 0) != 0 ||
...
bool panoramix_launch(panoramix_t *panoramix)
{
    ...
    if (v_infos == NULL)
        return false;
    if (pthread_create(&druid_thread, NULL, &druid_routine, panoramix) != 0)
        return false;
    for (unsigned int i = 0; i < panoramix->nb_villagers; i++) {
        v_infos[i].panoramix = panoramix;
        v_infos[i].i = i;
        if (pthread_create(&villager_threads[i], NULL,
                &villagers_routine, &v_infos[i]) != 0)
            return false;
    }
...
bool panoramix(args_t *args)
{
    ...
    if (panoramix_init(&panoramix, args) == false)
        return false;
    if (panoramix_launch(&panoramix) == false)
        return false;
